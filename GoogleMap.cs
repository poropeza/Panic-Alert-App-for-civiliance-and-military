...
public class GoogleMap : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //tengo que hacer una llamada a la base de datos para obtener todos los delitos registrados y calcular la distancia haversine para determinar si dicha 
        //ubicación está dentro del radio del usuario para que se pueda desplegar.

        AgregarMarcador();//este tiene que ir dentro del ciclo
    }
    ...
    void AgregarMarcador()
    {
        ...
        markers[0].locations[0].latitude = 18.909299f;
        markers[0].locations[0].longitude = -99.2129802f;
    ...
    IEnumerator _Refresh()
    {
        ...
        RawImage rendi = this.gameObject.GetComponent<RawImage>();
        rendi.material.mainTexture = new Texture2D(size, size, TextureFormat.DXT1, false);

        while (!req.isDone)
            yield return null;
        if (req.error == null)
            req.LoadImageIntoTexture((Texture2D)rendi.material.mainTexture);
    }
